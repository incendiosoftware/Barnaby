/**
 * Operation batch row - collapsible list of tool operations.
 */
...
export interface TimelineOperationBatchRowProps {
  batchKey: string
  units: TimelineUnit[]
  batchColor: string
  batchOpen: boolean
...
export const TimelineOperationBatchRow = React.memo(function TimelineOperationBatchRow({
  batchKey,
  units,
  batchColor,
  batchOpen,
  onToggle,
  formatToolTrace,
}: TimelineOperationBatchRowProps) {
  ...
  const renderItem = (u: TimelineUnit) => {
    const rawText = u.body.replace(/\s*\n+\s*/g, ' | ').trim()
